...
void BLEManager::startBeacon(const String& url) noexcept {
  ...
  if (encodedUrl.length() > Configuration::MAXIMUM_BLE_BEACON_ENCODED_URL_LENGTH) {

    DEBUG_PRINTF("Eddystone beacon URL too long after encoding: %d bytes (max %zu)\n",
                 encodedUrl.length(), Configuration::MAXIMUM_BLE_BEACON_ENCODED_URL_LENGTH);
  ...
  serviceData += static_cast<char>(eddystoneUrlFrameType);
  /**
