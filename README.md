...
#### ESP32 Platform Libraries (Bundled)
...
Alternatively, you can add libraries manually to your `lib` folder if you're using PlatformIO or another build system.
...
1. **[Museum Alert API](https://github.com/humana-fragilitas/museum-alert-api)** - AWS CDK infrastructure project that provides:
   ...
   **After deploying the Museum Alert API Infrastructure:**
   - **find the configuration**: look for the [`ArduinoSketchConfiguration`](https://github.com/humana-fragilitas/museum-alert-api?tab=readme-ov-file#configuration-output) output in your CDK deployment console;
   - **copy the complete configuration**: the deployment output provides a ready-to-copy `IOT_CORE_ENDPOINT` C++ constant expression;
   - **update configuration file**: replace the placeholder configuration in [`config.h`](https://github.com/humana-fragilitas/museum-alert-sketch/blob/main/config.h#L284).

2. **[Museum Alert Desktop](https://github.com/humana-fragilitas/museum-alert-desktop)** - Cross-platform Angular Electron desktop application suitable for provisioning, configuring, and testing the Museum Alert Sensor (MAS). The software most notably provides:
...
1. Plug your Museum Alert Sensor (MAS) into your computer using a USB-C cable;
...
- **activation**: comment out `#define RELEASE_BUILD` in [`config.h`](config.h#L19) (line 19).

**Release Mode:**
- **disables logging**: no debug output, optimized for production use;
- **fast startup**: no initialization delays, immediate operation;
- **activation**: uncomment `#define RELEASE_BUILD` in [`config.h`](config.h#L19) (line 19).

#### Compilation Options

**Using Arduino IDE:**
1. open the sketch file (`.ino`) from this repository;
2. modify the `#define RELEASE_BUILD` line in `config.h` as needed (see build modes above);
3. click the **Upload** button (right arrow icon);
4. wait for the sketch to compile and upload;
5. once complete, the board will automatically reset and begin running the sketch.
...
**Note**: Replace `/dev/cu.usbmodem*` with your actual port (use `arduino-cli board list` to find the correct port).
...
| **SET_PROVISIONING_CERTIFICATES** | 0 | `CONFIGURE_CERTIFICATES` | `{"tempCertPem": "...", "tempPrivateKey": "...", "idToken": "..."}` | Provides AWS IoT temporary provisioning certificates and Cognito user session token |
| **RESET** | 3 | `FATAL_ERROR` | None | Forces device reset and storage wipe |

#### USB Response Messages

The device sends messages and responses in JSON format wrapped with `<|` and `|>` markers:
...
**Note:** the correlation id property ```cid``` is only returned when the message is a response to a client-initiated request.
